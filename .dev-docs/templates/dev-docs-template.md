...
*A high-level technical overview of the project, its goals, and its architecture. This should provide enough context for a new developer to understand the system's purpose and how its major components interact.*
...
*Detailed instructions for setting up the development environment, installing dependencies, and getting the project running.*
...
4.  **Build/Compile (if applicable):**
    *Instructions from `./dev-docs/project-specifics.md` for building the project.*
    ```bash
    # Example
    make build
    ```
...
- **Development Mode:**
  ...
  ```
- **Running Tests:**
  *Instructions from `./dev-docs/project-specifics.md` for running unit/integration tests.*
  ```bash
  # Example
  pytest tests/
  ```
...
*Detailed documentation for each significant module, including its purpose, public API, and key internal components. This section should directly leverage information from `system_design_specification.md` and `ticket_template.md`.*
...
*A more in-depth look at specific architectural decisions, data flows, and design patterns relevant to developers. This should directly reference and elaborate on `system_design_specification.md`.*
...
*Instructions for developers wanting to contribute to the codebase.*
...
*Guidance for maintaining the system and diagnosing common issues.*
...
*A brief summary of recent changes, linking to the main `CHANGELOG.md` for full details.*
